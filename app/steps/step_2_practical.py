...
def render():
    render_login_info()
    if 'practical_page' not in st.session_state:
        st.session_state['practical_page'] = 0
    page = st.session_state['practical_page']
    ...
    if page == 0:
        ...
        with col_video:
            st.video("images/video-section-1-practical-requirements.mov")

    # Page 1: Architectural & Physical Space
    if page == 1:
    ...
    # Page 2: Daily Governance & Management
    if page == 2:
    ...
    # Page 3: Financial & Legal Expectations
    if page == 3:
    ...
    with col1:
        if st.button("← Back"):
            if page > 0:
                st.session_state['practical_page'] -= 1
            else:
                prev_step()

    def validate_practical_page(page, req):
        if page == 1:
            required = [
                req.get("desired_location"),
                req.get("physical_environment"),
                req.get("size_of_community"),
                req.get("regime_of_sharing"),
                req.get("private_dwelling")
            ]
            return all(x and (not isinstance(x, list) or len(x) > 0) for x in required)
        elif page == 2:
            required = [
                req.get("daily_management"),
                req.get("quiet_hours_importance"),
                req.get("guest_policy_importance")
            ]
            return all(x and (not isinstance(x, list) or len(x) > 0) for x in required)
        elif page == 3:
            required = [
                req.get("legal_structure"),
                req.get("budget_currency"),
                req.get("available_budget_purchase"),
                req.get("monthly_budget_rent")
            ]
    ...
    with col2:
        if st.button("Next →"):
            if page < 3:
                if validate_practical_page(page, req):
                    st.session_state['practical_page'] += 1
                    st.rerun()
                else:
                    st.warning("Please fill in all required fields before continuing.")
            else:
                if validate_practical_page(page, req) and _validate(req):
                    # Save answers to CSV
