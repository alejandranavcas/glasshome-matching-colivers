...
def render():
    render_login_info()
    if 'lifestyle_page' not in st.session_state:
        st.session_state['lifestyle_page'] = 0
    page = st.session_state['lifestyle_page']
    ...
    # Page 2: Lifestyle & Practical Needs
    if page == 2:
    ...
    with col1:
        if st.button("← Back"):
            if page > 0:
                st.session_state['lifestyle_page'] -= 1
            else:
                prev_step()

    def validate_lifestyle_page(page, req):
        if page == 1:
            required = [
                req.get("contact_with_neighbours"),
                req.get("mix_of_household"),
                req.get("frequency_shared_activities"),
                req.get("communal_activities")
            ]
    ...
    with col2:
        if st.button("Next →"):
            if page < 2:
                if validate_lifestyle_page(page, req):
                    st.session_state['lifestyle_page'] += 1
                    st.rerun()
                else:
